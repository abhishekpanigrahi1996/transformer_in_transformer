...
class GPT2_Forward(nn.Module):
    ...
    def __init__ (self, \
        ...
        super(GPT2_Forward, self).__init__()
        self.config=config
        ...
        if  add_biases_prefixes: self.add_biases(block['ln_1'], diagonal=True )
        ...
        if  add_biases_prefixes: 
            self.add_biases(block['attn']['c_attn'], separate_QK=separate_QK, attention=True, projection=None)         
        ...
        if  add_biases_prefixes: self.add_biases(block['attn']['c_proj'])
        ...
        self.memory_locations += [self.ln_memory_index]
        if  add_biases_prefixes: self.add_biases(block['ln_2'], diagonal=True)
        ...
        if  add_biases_prefixes: 
            for k in range(self.num_mlp_modules):
                ...
                self.add_biases(block['mlp']['c_fc'], projection=wt_projection)      
                
                self.add_biases(all_zeros=True)  
                
                wt_projection = self.outer_projection_matrix_wts[k]
                self.add_biases(block['mlp']['c_proj'], projection=wt_projection, project_input=True, project_bias= self.num_mlp_modules) 
    ...
    #The forward function first copies the necessary prefix embeddings from the biases.
    #After each TinT layer, we store the prefix embeddings in a stack, which we re-use for backpropagation layers.
    #Also, we store the necessary activations for backpropagation from the non-prefix tokens in a stack.
    def forward(self, \
                hidden_states, \
                position_embeddings, \
...
class gpt2block_backward(nn.Module):
    ...
    def __init__(self, \
        ...
        super(gpt2block_backward, self).__init__()
    ...
    def forward(self, \
        ...
        for module in self.mlp_modules:
            ...
            #first copy the weights onto the prefix tokens 
...
class TinT_gpt(nn.Module):
    ...
    def __init__(self, \
        ...
        super(TinT_gpt, self).__init__()
        self.model_config=model_config
        self.config=config
        
        self.wte = nn.Embedding(self.model_config.vocab_size, self.model_config.hidden_size)
        self.wpe = nn.Embedding(self.model_config.max_position_embeddings, self.model_config.hidden_size)
        
        #set the memory indices for each layer.
        #These were decided by how much space is necessary to store necessary activations for backpropagation
        ...
        self.wte.weight = model_dict['transformer']['wte'].weight
        self.wpe.weight = model_dict['transformer']['wpe'].weight
        ...
        for layer in tqdm(range(self.n_layers), desc="Building initial Forward simulation modules"):
            ...
            module = GPT2_Forward(config, \
                                       model_config, \
                                       block=model_dict['transformer']['h'][str(layer)], \
        ...
        self.gradient_module = finalgradient_compute(config, \
                                                     model_config, \
                                                     block=model_dict['transformer']['ln_f'], \
                                                     add_biases_prefixes=True, \
                                                     ln_memory_index=self.ln_memory_index, \
                                                    )
        ...
        for for_back_iter in range(self.n_forward_backward):
            for layer in tqdm(range(self.n_layers-self.n_back_layers, self.n_layers), desc="Building Backward simulation modules for iteration "+str(for_back_iter + 1)):
                ...
                if (not self.config.reuse_backward_blocks) or for_back_iter == 0:
                    module = gpt2block_backward(config, \
                                                model_config, \
                                                ln_memory_index=self.ln_memory_index, \
                                                attn_memory_index=self.attn_memory_index, \
                                                linear_memory_index=self.linear_memory_index, \
    ...
    def forward(self, \
        ...
        if test_backward_pass:
            ...
            for for_back_iter in range(self.n_forward_backward):
                ...
                #print (lnf_memory_prefixes[0][:, 0] - gradient_prefix[0][:, 0])
                ...
                #                  desc="Backward and descent, Iteration " +str(for_back_iter+1)):
                    ...
                    #print (  [torch.amax( torch.abs( memory_prefix_stack[layer][i] - memory_prefixes[i] ) )  for i in range(len(memory_prefixes))] )
                    #exit(0)
                ...
                #print ( max([ torch.max( gradient_prefix[0][:, i] - lnf_memory_prefixes[0][:, i] ).item() for i in range(self.config.num_prefixes) ]))  
                #exit(0)
